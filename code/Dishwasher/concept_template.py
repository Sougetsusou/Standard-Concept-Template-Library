...
class Multilevel_Leg(ConceptTemplate):
    def __init__(self, has_top_part, top_size, top_bottom_offset, front_legs_size, rear_legs_size, legs_separation, num_legs, position = [0, 0, 0], rotation = [0, 0, 0]):
        ...
        n = int(num_legs[0])
        ox, oz = top_bottom_offset[0], top_bottom_offset[1]
        sx, sz = legs_separation[0] / 2, legs_separation[2] / 2
        sy_r = legs_separation[1] / 2
        yf = -top_offset - front_legs_size[1] / 2
        yr = -top_offset - rear_legs_size[1] / 2
        leg_specs = {
            1: [(front_legs_size, [ox,        yf,  oz       ])],
            2: [(front_legs_size, [ sx + ox,  yf,  oz       ]),
                (front_legs_size, [-sx + ox,  yf,  oz       ])],
            3: [(front_legs_size, [ sx + ox,  yf,  sz + oz  ]),
                (front_legs_size, [-sx + ox,  yf,  sz + oz  ]),
                (rear_legs_size,  [ox,        yr, -sz + oz  ])],
            4: [(front_legs_size, [ sx + ox,  yf,  sz + oz  ]),
                (front_legs_size, [-sx + ox,  yf,  sz + oz  ]),
                (rear_legs_size,  [ sy_r + ox, yr, -sz + oz ]),
                (rear_legs_size,  [-sy_r + ox, yr, -sz + oz ])],
        }
