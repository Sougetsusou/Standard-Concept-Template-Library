...
class Storagefurniture_body(ConceptTemplate):
    def __init__(self, size, back_size, left_right_inner_size, base_size, has_lid, lid_size, lid_offset,
        ...
        self.EACH_number_of_layer = [storagefurniture_layers_params[i * 5] for i in range(int(WHOLE_number_of_layer[0]) + 1)]
        self.EACH_layer_sizes = [storagefurniture_layers_params[i * 5 + 1: i * 5 + 3] for i in range(int(WHOLE_number_of_layer[0]) + 1)]
        self.EACH_layer_offset = [storagefurniture_layers_params[i * 5 + 3] for i in range(int(WHOLE_number_of_layer[0]) + 1)]
        self.EACH_interval_between_layers = [storagefurniture_layers_params[i * 5 + 4] for i in range(int(WHOLE_number_of_layer[0]) + 1)]
        ...
        for mesh_idx in range(int(sum(TOTAL_mesh_list[:3]))):
            if mesh_idx < TOTAL_mesh_list[0]:
                if mesh_idx < 2:
                    ...
                    tmp_mesh = Cuboid(size[1], left_right_inner_size[0], size[2], position=mesh_position)
                elif mesh_idx == 3:
                    mesh_position = [0, -(size[1] - base_size[0]) / 2, 0]
                    tmp_mesh = Cuboid(base_size[0], size[0] - 2 * left_right_inner_size[0], size[2], position=mesh_position)
                else:
                    mesh_position = [0, 0, (back_size[0] - size[2]) / 2]
                    tmp_mesh = Cuboid(size[1], size[0], back_size[0], position=mesh_position)
                ...
                if has_lid[0] == 1:
                    ...
                    tmp_mesh = Cuboid(lid_size[1], lid_size[0], lid_size[2], position=mesh_position)
                else:
                    pass
            else:
                ...
                tmp_mesh = Cuboid(WHOLE_layer_sizes[0], size[0] - 2 * left_right_inner_size[0], size[2], position=mesh_position)
        ...
        for space_idx in range(WHOLE_layer_mesh_number + 1):
            ...
            for mesh_idx in range(int(self.EACH_number_of_layer[space_idx])):
                ...
                tmp_mesh = Cuboid(_height, self.EACH_layer_sizes[space_idx][0], self.EACH_layer_sizes[space_idx][1], position=mesh_position)
                vertices_list.append(tmp_mesh.vertices)
                faces_list.append(tmp_mesh.faces + total_num_vertices)
                total_num_vertices += len(tmp_mesh.vertices)
        ...
        for additional_idx in range(self.number_of_additional_layers):
            ...
            tmp_additional_mesh = Cuboid(self.additional_layers_attributes[9 * additional_idx + 1],
    ...
    def __init__(self, size, inner_sizes, additional_legs_params, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        for mesh_idx in range(4):
            ...
            if mesh_idx < 2:
                ...
                tmp_mesh = Cuboid(size[1], inner_sizes[0], size[2], position=mesh_position)
            else:
                mesh_position = [0, 0, position_sign * (size[2] - inner_sizes[1]) / 2]
                tmp_mesh = Cuboid(size[1], size[0], inner_sizes[1], position=mesh_position)
            vertices_list.append(tmp_mesh.vertices)
            faces_list.append(tmp_mesh.faces + total_num_vertices)
            total_num_vertices += len(tmp_mesh.vertices)
...
class Regular_door(ConceptTemplate):
    def __init__(self, number_of_door, doors_params, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.door_size = [doors_params[i * 12: i * 12 + 3] for i in range(int(number_of_door[0]))]
        self.handle_size = [doors_params[i * 12 + 3: i * 12 + 6] for i in range(int(number_of_door[0]))]
        self.handle_offset = [doors_params[i * 12 + 6: i * 12 + 8] for i in range(int(number_of_door[0]))]
        self.door_rotation = [doors_params[i * 12 + 8] for i in range(int(number_of_door[0]))]
        self.door_offset = [doors_params[i * 12 + 9: i * 12 + 12] for i in range(int(number_of_door[0]))]
        ...
        for door_idx in range(int(self.number_of_door[0])):
            for mesh_idx in range(2):
                if mesh_idx == 0:
                    ...
                    tmp_mesh = Cuboid(self.door_size[door_idx][1], self.door_size[door_idx][0], self.door_size[door_idx][2], position=mesh_position, rotation=mesh_rotation)
                else:
                    mesh_rotation = [0, self.door_rotation[door_idx], 0]
                    mesh_position = [
                        ...
                        self.door_offset[door_idx][2] - self.handle_offset[door_idx][0] * np.sin(self.door_rotation[door_idx]) + self.handle_size[door_idx][2] / 2 * np.cos(
                            self.door_rotation[door_idx])]
                    tmp_mesh = Cuboid(self.handle_size[door_idx][1], self.handle_size[door_idx][0], self.handle_size[door_idx][2], position=mesh_position, rotation=mesh_rotation)
...
class Regular_drawer(ConceptTemplate):
    def __init__(self, number_of_drawer, drawers_params, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.drawer_size = [drawers_params[i * 20: i * 20 + 3] for i in range(int(number_of_drawer[0]))]
        self.bottom_size = [drawers_params[i * 20 + 3] for i in range(int(number_of_drawer[0]))]
        self.front_size = [drawers_params[i * 20 + 4: i * 20 + 7] for i in range(int(number_of_drawer[0]))]
        self.front_offset = [drawers_params[i * 20 + 7] for i in range(int(number_of_drawer[0]))]
        self.left_right_inner_size = [drawers_params[i * 20 + 8] for i in range(int(number_of_drawer[0]))]
        self.rear_front_inner_size = [drawers_params[i * 20 + 9] for i in range(int(number_of_drawer[0]))]
        self.number_of_handle = [drawers_params[i * 20 + 10] for i in range(int(number_of_drawer[0]))]
        self.handle_sizes = [drawers_params[i * 20 + 11: i * 20 + 14] for i in range(int(number_of_drawer[0]))]
        self.handle_offset = [drawers_params[i * 20 + 14: i * 20 + 16] for i in range(int(number_of_drawer[0]))]
        self.handle_separation = [drawers_params[i * 20 + 16] for i in range(int(number_of_drawer[0]))]
        self.drawer_offset = [drawers_params[i * 20 + 17: i * 20 + 20] for i in range(int(number_of_drawer[0]))]
...
class Regular_front_panel(ConceptTemplate):
    def __init__(self, number_of_frontPanel, frontPanel_params, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.frontPanel_size = [frontPanel_params[i * 6: i * 6 + 3] for i in range(int(number_of_frontPanel[0]))]
        self.frontPanel_offset = [frontPanel_params[i * 6 + 3: i * 6 + 6] for i in range(int(number_of_frontPanel[0]))]
        ...
        for frontPanel_idx in range(int(number_of_frontPanel[0])):
            ...
            tmp_mesh = Cuboid(self.frontPanel_size[frontPanel_idx][1],
