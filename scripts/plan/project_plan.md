...
`code/shared/` organized by semantic type:

```
code/shared/
  geometry_template.py      # primitives: Cuboid, Cylinder, Ring, Torus, ...  [already here]
  base_template.py          # ConceptTemplate base class                       [already here]
  utils.py                  # apply_transformation, etc.                       [already here]
  knowledge_utils.py        # SAMPLENUM, etc.                                  [already here]
  concept_templates_shared.py  # identical duplicates already consolidated     [already here]
  body_templates.py         # Simple_Cuboidal_Body, Hollow_Cuboidal_Body,
                            #   Multilevel_Body, Stacked_Cuboidal_Body, ...
  leg_templates.py          # Cuboidal_Leg, Regular_Leg, Star_Leg,
                            #   Multilevel_Leg, Offset_Multilevel_Leg, ...
  door_templates.py         # Cuboidal_Door, Sunken_Door, Roller_Door, ...
  handle_templates.py       # Torus_Handle, Ring_Handle, Trifold_Handle,
                            #   Cylindrical_Handle, Cuboidal_Handle, ...
  cover_templates.py        # Fourfold_Cover, Regular_Cover, Cylindrical_Cover, ...
  blade_templates.py        # Curved_Blade, Cusp_Blade, Regular_Blade, ...
  button_templates.py       # Controller_With_Button, Regular_Controller, ...
  tray_templates.py         # Flat_Tray, Drawer_Like_Tray, ...
  connector_templates.py    # Cuboidal_Connector, Cylindrical_Connector, ...
  screen_templates.py       # Regular_Screen, ...
  base_part_templates.py    # Cuboidal_Base, Cylindrical_Base, Round_Base, ...
  wheel_templates.py        # Standard_Wheel, ...
  window_templates.py       # Symmetrical_Window, Asymmetrical_Window, ...
  spout_templates.py        # Curved_Spout, Straight_Spout, ...
  switch_templates.py       # Lever_Switch, Round_Switch, Rotary_Switch, ...
  ... (~20 files total)
```
...
**Deliverable:** all classes in shared/, all 39 visualize.py still run correctly.
...
from body_templates import Hollow_Cuboidal_Body
from leg_templates import Cuboidal_Leg
from cover_templates import Regular_Cover, Fourfold_Cover
```

**Write structured docstrings** for every part class in shared/:
```python
